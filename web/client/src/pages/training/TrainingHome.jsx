...
export default function TrainingHome() {
  ...
  const cet = ladder.find((c) => c.key === 'CET');
  const qei = ladder.find((c) => c.key === 'QEI');
  const cat = ladder.find((c) => c.key === 'CAT');
  const seniorTech = ladder.find((c) => c.key === 'SENIOR_TECH');
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="text-secondary" style={{ marginTop: '0.25rem' }}>
          Elevator certification paths
        </p>
      </div>

      <div className="stack">
        {/* Main track: CET → QEI → SENIOR_TECH */}
        {cet && <CertCard cert={cet} onNavigate={goTo} />}
        ...
        {/* QEI */}
        {qei && <CertCard cert={qei} onNavigate={goTo} />}
        ...
        {/* SENIOR_TECH */}
        {seniorTech && <CertCard cert={seniorTech} onNavigate={goTo} />}
          ...
          <>
            <div style={{ marginTop: '1rem' }}>
              <h3 style={{ textAlign: 'center', marginBottom: '0.75rem', color: '#A0A0A8', fontSize: '0.875rem' }}>
                Parallel Track (always available)
              </h3>
              <CertCard cert={cat} onNavigate={goTo} />
            </div>
          </>
        )}
      </div>
